...
var topicToPublishTo="tyAccel";
var topicToSubscribeTo="tyController";
//var characteristicOfInterest = ['2019'];
var notifChars = ['2019'];	// Array of UUID's
var charDescForNotify = "button press";
var intervalBetweenReads = 5000;
var state = 0;
const mariadb = require('mariadb/callback');
const conn = mariadb.createConnection({host:"localhost",user:"dit",password:"dit",database:"Accelerometer"});
var mongodb=require('mongodb');
var mqtt=require('mqtt');
var mongodbClient=mongodb.MongoClient;
var mqttClient = mqtt.connect('mqtt://broker.mqttdashboard.com');
var mongodbURI = 'mongodb://localhost:27017/mqtt_SensorData';
...
function discoverDeviceEventHandler(peripheral) { //event handler callback function 
	...
	*/
	if (peripheral.uuid==peripheralOfInterestUUID) {
		...
		mongodbClient.connect(mongodbURI, {useUnifiedTopology: true }, setupCollectionAndMQTT);
...
function discoverCharsCallAccel(error, characteristics) { //this will be executed when the discoverCharacteristics request returns
	if (error) {
		...
		console.log('service ' + serviceOfInterestUuids2 + ' has the following characteristics:  ');
...
function readDataCallback1(error, data) { //this will be executed when the read request returns
	if (error) {
		...
		var acceelG;
		acceelG = (num * 0.061)/1000;
		acceelGFixed =  Math.round(acceelG * 100) / 100;
		
		mqttClient.publish(topicToPublishTo, " from device " + peripheralOfInterest +" The Acceleration Reading is in g :" + acceelGFixed);//publishes sensor value to the MQTT topic
		 
		console.log("Displaying G unit : " + acceelG); // display the int to see the value
		
		if (num < '1000' && num > '10')// this if statemt is when the accel moving very fast to show that it dropping if the range from around 200 to 2000 that when the device dected it itself dropped 
		{
		...
		conn.query(
				"CREATE TABLE accelReading (" +
				"sensor_ID int NOT NULL AUTO_INCREMENT PRIMARY KEY,"+
				"device_ID VARCHAR(50) NOT NULL," +
				"AccelG FLOAT NOT NULL," +
				"AccelGMillsec FLOAT NOT NULL," +
				"AcccelRawValue MEDIUMINT NOT NULL," +
				"ts DATETIME NOT NULL)",	
			  err => {
				conn.query("INSERT INTO accelReading(device_ID,AccelG,AccelGMillsec ,AcccelRawValue, ts) value (?,?,?,?, now())", [peripheralOfInterestUUID,acceelGFixed,acceelG,num], (err, res) => {
				  if (err) throw err;
				  console.log(res);
...
function messageEventHandler(topic, message , packet) { 
   ...
   peripheralGlobal.discoverServices(['a000'], discoverServicesCallback); // call the discoverServicesCall back with a paramter of a000 to turn off LED
   } else if ( message == 'todayLog'){
	  conn.query("select * from accelReading ORDER BY sensor_ID DESC LIMIT 20 ", (err, res,meta) => {
	  if (err) throw err;

	 for (var i in res) {
			console.log(" From Device "+res[i].device_ID+ " Acceleration Value Reading From Accelerometer in G " + res[i].AccelG + " at " + res[i].ts);
			mqttClient.publish(topicToPublishTo," From Device "+res[i].device_ID+" Acceleration Value Reading in G " + res[i].AccelG + " at " + res[i].ts);
	...
	}else if ( message == 'allsLog'){
	  conn.query("select * from accelReading ORDER BY sensor_ID DESC LIMIT 50", (err, res,meta) => {
	  if (err) throw err;
	  
	  for (var i in res) {
			console.log("Acceleration  in G: " + res[i].AccelG + " Acceleration  in Gmills: "+res[i].AccelGMillsec+ " AcccelRawValue: "+ res[i].AcccelRawValue+" at " + res[i].ts);
			mqttClient.publish(topicToPublishTo," From Device "+res[i].device_ID+" Acceleration  in G: " + res[i].AccelG + " Acceleration in Gmills: "+res[i].AccelGMillsec+ " AcccelRawValue: "+ res[i].AcccelRawValue+" at " + res[i].ts);
        }
	  
	  //log : { res.Rows: 1, res.Id: 1, }
		});	
		
	}else if ( message == 'mqttLog'){
		
	  var MongoClient = mongodb.connect(mongodbURI, {useUnifiedTopology: true },findQueryMessage );

	}else if ( message == 'showTable'){
	  conn.query("select * from accelReading ", (err, res,meta) => {
	  if (err) throw err;
		console.log(res);
        });			
...
function setupCollectionAndMQTT(err, client) {
  if(err) throw err;
  var db=client.db('mqtt_SensorData');
  collection=db.collection("accel_sensorData");
  mqttClient.subscribe(topicToPublishTo);
  console.log("waiting for message for topic: "+topicToPublishTo);
...
function insertMessageInDB(topic, payload) {
    ...
    var key = topicToPublishTo;
	console.log("inserting message in database");

  var fromMqttObj = { value:payload.toString(), when:new Date() };
...
function findQueryMessage(err, client) {
  if (err) throw err;
  //var query = { value: 'from device TyBle The Acceleration Reading is in g :2.26' };
  var db=client.db('mqtt_SensorData');
  db.collection("accel_sensorData").find({}).limit(30).sort({$natural:-1}).toArray(function(err, result)
{
    if (err) throw err;
    if (!result.length) { 
        console.log("no matching entries");
    } else {
        console.log("there are " + result.length + " matching entries");
       mqttClient.publish(topicToPublishTo,"there are " + result.length + " matching entries");
        console.log(result);
    }
